...
$contact_map = '<div id="#contact-map-wrap"><googlemap>' . $mapContent . '</googlemap></div>';
  ...
  $mapCode = '   
   $("googlemap").each(function(){                        
    var embed ="<iframe width=\'100%\' height=\'360px\' frameborder=\'0\' scrolling=\'no\'  marginheight=\'0\' marginwidth=\'0\'  src=\'https://maps.google.com/maps?&amp;q="+ encodeURIComponent( $(this).text() ) +"&output=embed&z='.theme_get_setting('contact-map-zoom', 'pika').'\'></iframe>";
                                $(this).html(embed);                            
   });   ';
...
$header_line = " #header_line { background-image: url(" . base_path() . drupal_get_path('theme', 'pika') . "/images/" . theme_get_setting('header_line_bg', 'pika').");}";
}
...
if (theme_get_setting('layout_mode', 'pika') == 'boxed') {
 ...
 if ($layout_background == "layout_image") {
     $site_bg_preview = "body {
		background: url('". base_path() .  substr($site_bg_preview_img, 1). "') no-repeat center center fixed; 
  -webkit-background-size: cover;
  -moz-background-size: cover;
  -o-background-size: cover;
  background-size: cover;
 
		filter: progid:DXImageTransform.Microsoft.AlphaImageLoader(src='" . base_path() . substr($site_bg_preview_img, 1) . "', sizingMethod='scale'); 
		-ms-filter: progid:DXImageTransform.Microsoft.AlphaImageLoader(src='" . base_path() . substr($site_bg_preview_img, 1) . "', sizingMethod='scale');  }";
    }
